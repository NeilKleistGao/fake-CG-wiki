...
通常这个积分域我们会选择为 $[-\pi, \pi]$ 。唯一的例外是当基函数取1的时候。此时的积分值为周期长度 $2\pi$ ，正好是 $a_n$ 系数分母的两倍。所以我们不妨认为 $a$ 为 $a_0$ ，使用相同的公式计算：

$$
f(x) = \frac{a_0}{2} + \sum_{n = 1}^N (a_ncos(2\pi \frac{n}{N}x) + b_nsin(2\pi \frac{n}{N}x))
$$
 
至此，我们已经得到了函数的展开式。但是这个式子依然不够简洁。我们不妨利用三角公式对其进行化简。我们希望把 $a_ncos + b_nsin$ 写为 $A(cos\times cos + sin\times sin)$ 的形式，
这样就能将式子压缩到一个 $cos$ 函数中。为此，我们在式子中提出一个 $A_n = \sqrt {a_n^2+b_n^2}$ ，令 $\phi_n = arctan2(b_n, a_n)$ ，
我们得到 $a_ncos(2\pi \frac{n}{N}x) + b_nsin(2\pi \frac{n}{N}x) = A_n (cos(\phi_n)cos(2\pi \frac{n}{N}x) + sin(\phi_n)sin(2\pi \frac{n}{N}x))$ 。
最终我们化简的结果为：

$$
f(x) = \frac{A_0}{2} + \sum_{n = 1}^N A_ncos(2\pi \frac{n}{N}x - \phi_n)
$$

即便式子已经被化简，我们需要做的工作依然没有变化。为了得到 $A_n$ 和 $\phi_n$ ，我们还是需要求出 $a_n, b_n$ 。
为此，我们使用欧拉公式，将整个函数放到复平面上。因为 $cosx = \frac{e^{ix} + e^{-ix}}{2}$ ，
所以 $cos(2\pi \frac{n}{N}x - \phi_n) = \frac{1}{2}e^{i(2\pi \frac{n}{N}x - \phi_n)} + \frac{1}{2}e^{-i(2\pi \frac{n}{N}x - \phi_n)}$ 。
将所有的 $\phi$ 项提出，得到： $(\frac{1}{2}e^{-i\phi_n})e^{\frac{2\pi nx}{N}} + \overline{(\frac{1}{2}e^{-i\phi_n})}e^{\frac{-2\pi nx}{N}}$ ，
并将其与前面的 $A_n$ 项合并，我们最终的傅立叶级数被化简为：

$$
f(x) = \sum_{n = -N}^N c_ne^{i\frac{2\pi nx}{N}}
$$

其中， $c_n = \frac{1}{N}\int_N f(x)e^{-i\frac{2\pi nx}{N}}dx$ 。

我们通常会只保留系数部分，也就是说，我们对于函数在频率变化时，每个分量的大小变化感兴趣。我们只保留前面系数的部分，并认为任意函数的周期不再是 $2\pi$ 而是无穷，
我们便可以得到傅立叶变换的公式：
...
同样我们可以使用逆变换将函数从频率的形式变回到时间（或者空间） 的形式：

$$
f(x) = \int_{-\infty}^{\infty}f(\xi)e^{2\pi ix\xi}d\xi
$$

对于以频率为参数的函数，我们称其所在的域为频域；相反，我们称其所在的域为时域。傅立叶变换是将函数从时域到频域的变换。

## 走样产生的原因
现在，走样产生的原因已经非常明显了：由于采样的速度无法跟上改变的速度（也就是频率不足），导致我们无法恢复出正确的函数：

![Screenshot from 2021-10-20 08-08-51.png](https://i.loli.net/2021/10/20/hVRpgH1C2aFMKt8.png)

如图，这是一个 $sin(2\pi x)$ 的图像。假如我们所做的采样频率很低（图上为1），而函数本身的频率远超出这个值（ $2\pi$ ），你就会发现我们无法恢复出原来的函数。
根据采样点，我们只能恢复出一个常数函数，而事实上这是一个三角函数。

因此，走样经常发生在图像的边缘部分（这部分频率比图像中心要更高），通常会产生锯齿状的效果。对于解决走样，我们可以考虑如下的角度：

+ 采用低通滤波。这样的滤波器会过滤掉高频的信号，从而使得采样可以跟上信号频率。代价是边界会变得更加模糊
+ 增加采样，这是最直接最有效的方案，但是增加采样的代价往往是比较高的
